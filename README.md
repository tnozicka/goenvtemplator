...
There are a few built in functions as well:
  ...
  * `requires (env "ENV_NAME")` - Renders an error if environments variable does not exists. If it is equal to empty string, returns empty string.  `{{ requires (env "TIMEOUT_MS) }}`
